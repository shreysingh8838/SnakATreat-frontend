...
const Navbar = (props) => {
    ...
    return(
<StyledNavbar>
  <nav className="bg-indigo-900 dark:bg-gray-800">
    <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-10">
      <div className="flex items-center justify-between h-16">
        <div className="flex items-center">
          <div className="flex-shrink-5">
          <NavLink to="/">
            <img className="h-20 w-29 " src={logo13} alt="logo"/>
            {/* <h1 > SNAK -A- </h1><h1 className = "text-blue-300"> TREAT </h1> */}
          </NavLink>
          </div>
                  {/* //to change or copy */}
          <div>
            <span onClick={ () => setTheme(colorTheme)} className="bg-white rounded-full mr-3 text-white shadow-lg block cursor-pointer">
              {colorTheme === "light" ? 
                (<p className="px-5 py-2 rounded-full text-sm font-medium text-black hover:text-white hover:bg-gray-300 hover:focus-outline-none"> 
                <img src={eye} alt="slash" className="h-6 w-10 px-1 cursor-pointer fill-current"/></p>) 
                :(<p className="px-5 py-2 rounded-full text-sm font-medium text-black hover:text-white hover:bg-gray-700"> 
                <img src={eyeSlash} alt="slash" className="h-6 w-10 px-2 cursor-pointer fill-current"/> </p>)}
            </span>
          ...
          <div className="hidden md:block">
            <div className="flex items-baseline space-x-4">
              ...
              <span className="bg-red-700  text-white text-xs ml-4 mt-1 sm:-ml-0 sm:-mt-2  inline-block px-1  absolute rounded-full">{ props.cartItems.length} </span>
              }
              </NavLink>
            </div>
          </div>
        ...
        <div className="hidden md:block">
          <div className="ml-4 flex items-center md:ml-6">
            <button className="bg-red-700 p-1 rounded-full text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
