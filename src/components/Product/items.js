...
const Items = (props) => {
            ...
            <div className="relative px-4 -mt-16">
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    <div className="flex items-baseline">
                        <span className="inline-block dark:bg-green-200 bg-red-200 text-teal-800 text-xs px-2 rounded-full uppercase font-semibold tracking-wide">New</span>
                         <div className="ml-2 dark:text-gray-600 text-red-600 text-xs uppercase font-semibold tracking-wide">
                            {product.name}
                        </div>
                    </div>
                     <div className="mt-1 dark:text-gray-600 text-red-600">
                        ₹{product.price}
                        <span className="dark:text-gray-600 text-red-600 text-sm"> / plate</span>
                    </div>
                    <div className="mt-2 flex items-center">
                        <Rating rating={product.rating}/>
                        <span className="ml-2 dark:text-gray-600 text-red-600 text-sm">{product.numReviews} review{product.rating && Number(product.numReviews) > 1 ? 's': ''}</span>
                    </div>
                </div>
            </div>
   ...
   !props.paginating && !props.willPaginate &&
    <p className="flex dark:text-gray-600 text-red-600 mx-4 w-full flex-row justify-center items-center text-center mb-4">
